...
import CategoriaNova from "./New";

const BASE_URL = "http://localhost:8000/get_all_categorias/";
...
const CategoriaList = (props) => {
  ...
  const [openModal, setOpenModal] = useState(false);
    ...
    const fetchOrcamentsData = async () => {
      try {
        ...
        console.log("data.categorias: ", data.categorias)
        setTableContent(data.categorias);
        setLoading(true);
      } catch (error) {
  ...
  return (
    <div className="dashboard-container">
      ...
      >
        <div>
          <CategoriaNova closeModal={handleCloseModal} />
        </div>
      </Modal>
