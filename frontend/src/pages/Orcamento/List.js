...
const BASE_URL = "http://localhost:8000/get_all_orcamentos/";

const OrcamentoList = (props) => {
  ...
  const [openModal, setOpenModal] = useState(false);
    ...
    const fetchOrcamentsData = async () => {
      try {
        ...
        console.log("data.orcamentos: ", data.orcamentos)
        setTableContent(data.orcamentos);
        setLoading(true);
      } catch (error) {
                  ...
                  {currentItems?.map((content, ind) => {
                    return (
                      ...
                      >
                        ...
                        <td>{content.qtd_itens}</td>
                        <td>R$ {content.valor_orc.toFixed(2)}</td>
                        <td>{content.descri_orc}</td>
                      </tr>
                    );
                  })}
                </tbody>
